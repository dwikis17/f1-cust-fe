...
export async function POST(request: Request) {
	if (!apiBaseUrl) return Response.json({ error: { code: "LOCATION_NOT_CONFIGURED", message: "Address search is not configured" } }, { status: 503 });
	let body: unknown;
	try {
	...
	try {
		const upstream = await fetch(`${apiBaseUrl}/api/locations/search`, {
		...
		return Response.json({ error: { code: "LOCATION_UPSTREAM_ERROR", message: "Address search is temporarily unavailable" } }, { status: 502 });
	}
}
